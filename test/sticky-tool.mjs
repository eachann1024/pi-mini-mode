...
try {
  ...
  assert.match(screen().join('\n'), /当前活动：bash npm test/, 'real SGR click reveals live details in the viewport');
  assert.match(docRows().join('\n'), /LIVE_END/, 'expanded activity is not truncated to the heading width');
  ...
  assert.match(screen().join('\n'), /当前活动：bash npm run check/, 'expanded live activity refreshes');
  click(3, childY() - scroll.scrollTop); await paint();
  assert.doesNotMatch(docRows().join('\n'), /当前活动/, 'second real click collapses a running SubAgent');
  click(3, childY() - scroll.scrollTop); await paint();
